...
def run(param1, param2, param3):
    ...
    # Enter code here
    #
    tau_p, tau_d, tau_i = param1, param2, param3
    int_CTE = 0
    prev_CTE = myrobot.y
    history = []
    for i in range(1000):
...
#        print [myrobot.x, myrobot.y], ';',
        history.append([myrobot.x, myrobot.y])
        
    import matplotlib.pyplot as mtpl
    mtpl.figure()
    mtpl.hold(True)
    mtpl.plot([p[0] for p in history], [p[1] for p in history], label='test')
    mtpl.legend()
    mtpl.show()
# Call your function with parameters of (0.2, 3.0, and 0.004)
#run(0.2, 3.0, 0.004) # correct case
#run(0., 3.0, 0.004) # no tau_p, 
#run(0.2, 0., 0.004) # no tau_d, diff
run(0.2, 3.0, 0.)   # no tau_i, integral
