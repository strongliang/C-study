...
class robot:
    ...
    def move(self, motion): 
        ...
        if abs(beta) <0.001:
            ...
            result.x = cx + sin(theta + beta) * R
            result.y = cy - cos(theta + beta) * R
#            steer_angle = random.gauss((theta + beta) % (2 * pi), self.steering_noise)
            steer_angle = random.gauss((beta) % (2 * pi), self.steering_noise) + theta
            result.orientation = steer_angle % (2 * pi)
    ...
    def sense(self, noise=1): #do not change the name of this function
        ...
        for i in range(len(landmarks)):
            ...
            theta = self.orientation
            x = atan2(delta_y, delta_x) - theta
            if noise == 1:
                x += random.gauss(0, self.bearing_noise)
            x = x % (2 * pi)
...
#    print check_output(final_robot, estimated_position)
    if check_output(final_robot, estimated_position) == True:
#        print 'True'
        counter[0] += 1.
    else:
#        print 'False'
...
print counter
print "success rate is: ", counter[0]/(counter[1]+counter[0])
