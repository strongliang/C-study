...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    #
    #
    dim = N+num_landmarks # dimension
    omega = [[0 for i in range(dim)] for j in range(dim)]
    xi = [[0, 0] for i in range(dim)]
    ...
    for i in range(N-1): # going through the steps
        motion = data[i][1]
        
        delta_x = [[0 for cell in range(2)] for row in range(dim)]
        delta_o = [[0 for cell in range(dim)] for row in range(dim)]
        
        # motion update
        delta_x[i][0] += -motion[0] / motion_noise
        delta_x[i][1] += -motion[1] / motion_noise
        delta_x[i+1][0] += motion[0] / motion_noise
        delta_x[i+1][1] += motion[1] / motion_noise
        
        delta_o[i][i] += 1 / motion_noise
        delta_o[i][i+1] += -1 / motion_noise
        delta_o[i+1][i] += -1 / motion_noise
        delta_o[i+1][i+1] += 1 / motion_noise
        
#        print 'step', i+1
        for j in range(len(data[i][0])): # going thru the landmark measurements
            measurement = data[i][0][j]
#            print 'landmark ', data[i][0][j][0], ' : ', [measurement[1], measurement[2]]

            land_idx = N + measurement[0]  # landmark index     
            
            # measurement update
            delta_x[i][0] += -measurement[1] / measurement_noise # update x
            delta_x[i][1] += -measurement[2] / measurement_noise # update y
            delta_x[land_idx][0] += measurement[1] / measurement_noise
            delta_x[land_idx][1] += measurement[2] / measurement_noise
            
            delta_o[i][i] += 1 / measurement_noise
            delta_o[i][land_idx] += -1 / measurement_noise
            delta_o[land_idx][i] += -1 / measurement_noise
            delta_o[land_idx][land_idx] += 1 / measurement_noise
        
#        print delta_x
#        print delta_o
        
        Omega += matrix(delta_o)
        Xi += matrix(delta_x)
#        print '==================='
