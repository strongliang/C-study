...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    dim_exp = dim + 2 # each time we expand by unit of 2
    ...
    for k in range(len(data)):
        ins_range = [0, 1] + range(4, dim_exp) # insert at row 2 col 2 thus [0, 1, 4, 5...dim_exp]
        take_range = range(2, dim_exp) # take all but row 0, 1 and/or col 0, 1
        
        Omega = Omega.expand(dim+2, dim_exp, ins_range, ins_range)
        Xi = Xi.expand(dim_exp, 1, ins_range, [0])
        Omega.show()
        Xi.show()
        ...
        for i in range(len(measurement)):
            # m is the index of the landmark coordinate in the matrix/vector
            m = 2 * (2 + measurement[i][0]) # '2+' because there are two steps in prime
            
            # n is the index of the robot pose in the matrix/vector
            n = 0 # it's always 0 because we only keep the most recent one
        ...
        Omega_pri = Omega.take(take_range, take_range)
        Xi_pri = Xi.take(take_range, [0])
        
        A = Omega.take([0, 1], take_range)
        B = Omega.take([0, 1], [0, 1])
        C = Xi.take([0, 1], [0])
        
        Omega = Omega_pri - A.transpose() * B.inverse() * A
        Xi = Xi_pri - A.transpose() * B.inverse() * C
