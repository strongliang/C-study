...
def plan(road, lane_change_cost, init, goal): # Don't change the name of this function!
    ...
    #
    #
    delta = [[-1, 1 ], # left shift
             [ 0, 1], # go straight
             [ 1, 1 ]] # right shift

    delta_name = ['//', '==', '\\']
    
    value = [[99 for row in range(len(road[0]))] for col in range(len(road))]
...
test_road1 = build_road(15, [90, 80, 50, 10], obstacles=True, obstacle_prob=0.1)
test_road1[len(test_road1)-1][0] = 1
lane_change_cost1 = 1.0 / 50.0
test_init1 = [len(test_road1) - 1, 0]
test_goal1 = [len(test_road1) - 1, len(test_road1[0]) - 1]
